...
public class OpcionComidaService : IOpcionComidaService
{
    ...
    public OpcionComidaService(NutriAppDbContext context)
    {
        _context = context;
    }
    ...
    public async Task<ResultadoDieta<OpcionSeccionComidaDto>> CrearOpcionAsync(
        int nutricionistaId,
        int pacienteId,
        int dietaId,
        int comidaId,
    ...
    {
        var seccion = await ObtenerSeccionModificableAsync(
            nutricionistaId,
            pacienteId,
            dietaId,
            comidaId,
        ...
        if (seccion.Comida.Dieta.Estado == EstadoDieta.Archivada)
        {
            return Error<OpcionSeccionComidaDto>(
        ...
        {
            var opcionesPredeterminadas = await _context
                .OpcionesSeccionesComidas.Where(o =>
                    o.SeccionComidaId == seccionId && o.EsPredeterminada
                )
                .ToListAsync();
        ...
        var nuevaOpcion = new OpcionSeccionComida
        {
        ...
        _context.OpcionesSeccionesComidas.Add(nuevaOpcion);
        ...
        return new ResultadoDieta<OpcionSeccionComidaDto>
        {
            ...
            Datos = MapearOpcion(nuevaOpcion, new List<ItemOpcionComidaDto>()),
    ...
    public async Task<ResultadoDieta<List<OpcionSeccionComidaDto>>> ObtenerOpcionesAsync(
        int nutricionistaId,
        int pacienteId,
        int dietaId,
        int comidaId,
    ...
    )
    {
        var existe = await _context
            .SeccionesComidas.AsNoTracking()
            .AnyAsync(s =>
                s.Id == seccionId
                && s.ComidaId == comidaId
                && s.Comida.DietaId == dietaId
                && s.Comida.Dieta.PacienteId == pacienteId
                && s.Comida.Dieta.Paciente.NutricionistaId == nutricionistaId
            );
        ...
        var entidades = await _context
            .OpcionesSeccionesComidas.AsNoTracking()
            .Include(o => o.Items)
                .ThenInclude(i => i.Alimento)
            .Include(o => o.Items)
                .ThenInclude(i => i.Alternativas)
            .Where(o => o.SeccionComidaId == seccionId)
            .OrderBy(o => o.Orden)
            .ThenBy(o => o.Id)
            .AsSplitQuery()
            .ToListAsync();

        var opciones = entidades
            .Select(o =>
                MapearOpcion(
                    o,
                    o.Items.OrderBy(i => i.Orden)
                        .ThenBy(i => i.Id)
        ...
        return new ResultadoDieta<List<OpcionSeccionComidaDto>>
        {
            Exitoso = true,
    ...
    public async Task<ResultadoDieta<OpcionSeccionComidaDto>> ObtenerOpcionAsync(
        int nutricionistaId,
        int pacienteId,
        int dietaId,
        int comidaId,
    ...
    )
    {
        var opcion = await _context
            ...
            .Where(o =>
                ...
                && o.SeccionComida.Comida.Dieta.PacienteId == pacienteId
                && o.SeccionComida.Comida.Dieta.Paciente.NutricionistaId == nutricionistaId
            )
            .AsSplitQuery()
            .FirstOrDefaultAsync();
        ...
        return new ResultadoDieta<OpcionSeccionComidaDto>
        {
            ...
            Datos = MapearOpcion(opcion, items),
    ...
    public async Task<ResultadoDieta<OpcionSeccionComidaDto>> EditarOpcionAsync(
        int nutricionistaId,
        int pacienteId,
        int dietaId,
        int comidaId,
    ...
    {
        var opcion = await _context
            .OpcionesSeccionesComidas.Include(o => o.Items)
                .ThenInclude(i => i.Alimento)
            .Include(o => o.Items)
                .ThenInclude(i => i.Alternativas)
        ...
        if (opcion.SeccionComida.Comida.Dieta.Estado == EstadoDieta.Archivada)
        {
            return Error<OpcionSeccionComidaDto>(
                "No se puede modificar una dieta archivada.",
                TipoErrorDieta.Validacion
        ...
        if (dto.EsPredeterminada && !opcion.EsPredeterminada)
        {
            var otras = await _context
                .OpcionesSeccionesComidas.Where(o =>
                    o.SeccionComidaId == seccionId && o.Id != opcionId && o.EsPredeterminada
                )
                .ToListAsync();
        ...
        opcion.Nombre = dto.Nombre.Trim();

        opcion.Orden = dto.Orden;

        opcion.EsPredeterminada = dto.EsPredeterminada;

        opcion.Observaciones = dto.Observaciones?.Trim();
    ...
    public async Task<ResultadoDieta<ItemOpcionComidaDto>> CrearItemAsync(
        int nutricionistaId,
        int pacienteId,
        int dietaId,
        int comidaId,
    ...
    {
        var opcion = await _context
            .OpcionesSeccionesComidas.Include(o => o.SeccionComida)
                .ThenInclude(s => s.Comida)
                    .ThenInclude(c => c.Dieta)
                        .ThenInclude(d => d.Paciente)
            .FirstOrDefaultAsync(o =>
        ...
        if (opcion is null)
        {
            return Error<ItemOpcionComidaDto>("Opción no encontrada.", TipoErrorDieta.NoEncontrado);
        }
        ...
        if (alimento is null)
        {
            return Error<ItemOpcionComidaDto>("Alimento no encontrado.", TipoErrorDieta.Validacion);
        }
        ...
        if (dto.UnidadMedida != alimento.UnidadBase)
        {
            return Error<ItemOpcionComidaDto>(
        ...
        {
            OpcionSeccionComidaId = opcionId,
            ...
            Indicaciones = dto.Indicaciones?.Trim(),
        ...
        return new ResultadoDieta<ItemOpcionComidaDto>
        {
            ...
            Datos = MapearItem(item, alimento),
    ...
    // EDITAR ITEM
    // ==========================================
    public async Task<ResultadoDieta<ItemOpcionComidaDto>> EditarItemAsync(
        int nutricionistaId,
        int pacienteId,
        int dietaId,
        int comidaId,
    ...
    )
    {
        var item = await _context
            .ItemsOpcionesComidas.Include(i => i.Alternativas)
        ...
        if (item is null)
        {
            return Error<ItemOpcionComidaDto>("Ítem no encontrado.", TipoErrorDieta.NoEncontrado);
        }
        ...
        item.Indicaciones = dto.Indicaciones?.Trim();
    ...
    public async Task<ResultadoDieta<bool>> EliminarOpcionAsync(
        int nutricionistaId,
        int pacienteId,
        int dietaId,
        int comidaId,
    ...
    )
    {
        ...
        if (opcion is null)
        {
            return Error<bool>("Opción no encontrada.", TipoErrorDieta.NoEncontrado);
        }
        ...
        _context.OpcionesSeccionesComidas.Remove(opcion);
    ...
    public async Task<ResultadoDieta<bool>> EliminarItemAsync(
        int nutricionistaId,
        int pacienteId,
        int dietaId,
        int comidaId,
    ...
    {
        var item = await _context
            .ItemsOpcionesComidas.Include(i => i.OpcionSeccionComida)
                .ThenInclude(o => o.SeccionComida)
                    .ThenInclude(s => s.Comida)
                        .ThenInclude(c => c.Dieta)
            .FirstOrDefaultAsync(i =>
        ...
        _context.ItemsOpcionesComidas.Remove(item);
    ...
    private async Task<SeccionComida?> ObtenerSeccionModificableAsync(
        int nutricionistaId,
        int pacienteId,
        int dietaId,
        int comidaId,
    ...
    )
    {
        return await _context
            .SeccionesComidas.Include(s => s.Comida)
                .ThenInclude(c => c.Dieta)
                    .ThenInclude(d => d.Paciente)
    ...
    {
        var resultadoTotales = CalculadoraTotalesNutricionales.CalcularOpcion(opcion.Items);

        return new OpcionSeccionComidaDto
        {
            Id = opcion.Id,

            SeccionComidaId = opcion.SeccionComidaId,

            Nombre = opcion.Nombre,

            Orden = opcion.Orden,

            EsPredeterminada = opcion.EsPredeterminada,
    ...
    {
        var alimento = alimentoOverride ?? item.Alimento;
        ...
        return new ItemOpcionComidaDto
        {
            ...
            Alimento = alimento?.Nombre ?? string.Empty,
            ...
            CantidadAlternativas = item.Alternativas?.Count(a => a.Activa) ?? 0,

            Nutricion = alimento is null
                ? null
                : CalcularNutricion(item.Cantidad, item.UnidadMedida, alimento),
        };
    }
    ...
    private static NutricionCalculadaDto? CalcularNutricion(
        decimal cantidad,
    ...
    )
    {
        ...
        if (unidadMedida != alimento.UnidadBase)
        {
            return null;
        }

        var resultado = CalculadoraNutricional.Calcular(
            cantidad,
